...
- All org-scoped access control is enforced by NestJS (decorator + guard). Better Auth is used solely to establish _who the caller is_.
...
| 3   | **Invite discovery:** Signed magic link email _plus_ an in-app pending-invites inbox.                                                                                                                                                    |
...
- Partial unique index: `UNIQUE(organization_id, email) WHERE status = 'pending'` — guarantees at most one _live_ invite per (org, email). The duplicate-invite flow (flip old → `expired`, insert new) runs inside a single transaction so the partial index stays satisfied at every step.
...
apps/backend/src/organizations/
  ...
  organizations.service.ts      # Core org/membership/transfer/delete business logic
  members.controller.ts         # Member read/update-role/remove/leave
  members.service.ts
  invites.controller.ts         # Invite create/list/revoke/resend/accept/decline/preview + my-invites
  invites.service.ts            # Token generation + hashing + email dispatch + accept flow
  guards/
    org-context.guard.ts
  decorators/
    require-org-role.decorator.ts
    org-membership.decorator.ts
  dto/                          # Zod pipes (runtime schemas imported from @launchstack/api-interfaces)
```
...
**`organization.requests.ts`** exports Zod schemas _and_ inferred types:
